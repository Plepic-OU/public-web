...
| --- | --- |
| Crawl | `explore(); act(); verify();` typed once per gait cycle, three lines |
| Gather | lines fold up; survivor swaps to `{ explore → act → verify }`, cursor freezes solid |
...
| Rest | `while(task) { explore → act → verify }` — byte-identical to production — cursor blinking, 7s light wave |
