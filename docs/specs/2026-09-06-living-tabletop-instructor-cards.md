...
1. The tilt is off for team cards wherever the career slide covers them, at `(hover: hover) and (pointer: fine) and (min-width: 1024px)`, through `--tilt-enabled`. The slide covers the whole grid there, so the tilt would be invisible under it and the hover ink shadow would peek out above the panel's top edge. CSS owns the breakpoint and `js/tabletop.js` reads the computed value, so putting the tilt back is one token in one media block. The other reading is that the slide is the thing to drop, not the tilt; Kaido decides which hover state the team section keeps.
