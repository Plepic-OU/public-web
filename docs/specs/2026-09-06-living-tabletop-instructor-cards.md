...
> **Section 9 supersedes the card design in 5.3, 5.4 and 5.6.** Kaido rejected the trading card on 2026-09-08 and chose a photographic editorial column in its place. The mark, the tokens, the tilt and everything in 5.1, 5.2 and 5.5 stand. Read section 9 before you read section 5.
...
## 8. Resolved during the build

The questions the build had to answer to ship, the answer it took, and what reversing it costs.

1. **Settled by Kaido on 2026-09-08: the career slide stays.** His reason: it "allows to get a one-glance overview of instructors", which is a job the card cannot do and the tilt was never going to do. So the tilt is off for team cards wherever the slide covers them, at `(hover: hover) and (pointer: fine) and (min-width: 1024px)`, through `--tilt-enabled`. The slide covers the whole grid there, so the tilt would be invisible under it and the hover ink shadow would peek out above the panel's top edge. CSS owns the breakpoint and `js/tabletop.js` reads the computed value, so putting the tilt back is one token in one media block. The alternative reading, that the slide should be dropped in favour of the tilt, was considered and rejected.
2. The Card demo on `design-system.html` sits in a white `.panel`. The Cards guard sanctions exactly one `data-demo="panel-cream"` specimen on the page, so a second cream panel fails CI. The Card is a specimen like every other, not the cream variant demo.
3. All three cards carry `data-foil`. Foil on one card ranks three real people in public, which is the reason section 6 already rejects tiers and power numbers, and the reason does not weaken because the device is material instead of a number.
4. Kaido's live card uses `images/portraits/kaido.svg`, triangulated from `images/kaido.jpg`, the smiling shot the site carries today. `images/portraits/kaido-cutout.svg` is generated from the transparent `images/kaido.png` and committed beside it, so the other choice is one `src` swap with no pipeline run.
5. Eight tokens beyond the 5.1 list are declared in `:root`: `--tilt-perspective`, and the runtime channels `--tilt-enabled`, `--tt-lift`, `--tt-rx`, `--tt-ry`, `--tt-foil-x`, `--tt-foil-y` and `--mark-perspective`. The transform belongs to CSS so that the lift survives reduced motion and a module that bails, which leaves the modules writing custom properties per element and nothing else. The "every var() resolves" guard reads the stylesheet statically, so `var(--tt-rx)` inside the card transform fails CI unless `:root` declares the name. The browser needs the same declaration for a second reason: before the module writes anything, an undeclared channel makes the transform invalid at computed-value time, and the card renders flat with nothing in the console. Declaring each with its rest value is what makes the no-JS rendering the authored one.
6. `js/plepic-mark.js` and `js/tabletop.js` are registered in the `ASSETS` list of `scripts/cache-bust.mjs`. Without it a returning visitor runs the last deploy's modules against this deploy's stylesheet, which is the one failure that reaches only the people who already came once.
7. The cards stack at 600px, not at the 720px section 5.5 names. 601 to 768px keeps two columns, which is where the existing breakpoint sits and is shared with the nav, the menu toggle and the hero stats. Two 300px cards and their gap need 632px and a 700px viewport gives 652px, so the pair fits with room to spare; a single centred card there would spend half the width on margin. Moving the stack to 720px means a third media query, not an edit to the shared one.
8. The three cards are one height. `align-items: stretch` on the grid and a column layout inside the card put the spare height in the text box, so the foot plate sits on the bottom edge of every card. A day-job line that wraps, as Kaido's does, otherwise made one card 23px taller than the two beside it, and three heights read as an accident rather than as a deck. At the two-column width the third card is alone on its row and keeps its natural height, which is what a row of one means.
9. No card casts a shadow while the slide panel is up. The panel is exactly as wide as the grid, so the outermost card's 3px rest shadow fell in the 3px beside it and drew a dark hairline down the panel's right edge for the full height of the cards. Suppressing the shadow on the hovered card alone does not fix it, because it is the cards either side that show, so the rule is on every card in a hovered showcase. Measured after the fix: five dark pixels in a 6 by 360 strip beside the panel, all of them the panel's own corner antialiasing, against roughly a thousand before.
10. The hover wingbeat plays, and its handover still jumps. Both the beat and the breath animate `transform` on the same layer, and the later name in the `animation` list wins while it is in effect. The beat was named first, so the breath masked it for its whole run: swept across six arrival phases the wing opened to 13, 27, 34, 34 and 29 degrees, `--breath-open`, instead of the 62 of `--wingbeat-open`, and only a pointer landing in the first second of the cycle ever saw a beat. Naming the beat last fixes that at every phase, measured. What is left is the handover: a running breath keeps its clock and takes the beat's 1.1s delay retroactively, so the wing steps 6 to 34 degrees in a single frame at hover-in and again at hover-out. Removing that needs the two rotations to compose instead of compete, a hinge element carrying the breath with the layer carrying the beat, which adds a DOM node per wing. The PR that gives the header and footer the mark is the one that should do it, because at the 30px nav size the logo is the most hovered thing on the site. `tests/plepic-mark.spec.ts` holds the line that the beat still reaches `--wingbeat-open` at three arrival phases, since nothing in the suite could see the masking when it shipped.
11. The card sits in a seat. A transformed element is hit-tested against its transformed box, so a card that owned its own `:hover` fed itself: lift it 12px and a pointer resting 3px inside its top edge is outside it, hover drops, the card falls back, the pointer is inside again. Measured before the fix: 86 enter and leave events in two and a half seconds, and a set glyph that could never hold hover long enough to play a beat. `.tt-seat` is an untransformed wrapper that owns hover and the `data-tilt` hook; the card owns the transform and reads the channels by inheritance. After: one event. The same wrapper is why `js/tabletop.js` can now measure per frame, which is what fixes the tilt reading from an origin the page has scrolled out from under.

## 9. Superseded on 2026-09-08: the card is a photograph on a printed column

Kaido saw the built cards and rejected three things by name: "the distorted photos of instructors are ugly", "the background random triangle graphics is too much", "the black edges of the cards do not fit", and "these aspects of card design are a failure". He asked for a redesign shown as several directions and chose real photographs. Six directions were built as working comps, he chose "Editorial rule", then chose the "Graded set" colour treatment from five, and asked for the word instructor to stop appearing twice on every card. That is what ships.

**What this reverses.** Section 6 rejected "Print (duotone photo)" in favour of crystalline art for the card art. That decision is dead for the instructor cards. Crystalline stays where it belongs, the logo and the hero, which is where its geometry came from. `scripts/triangulate-portrait.mjs` and `images/portraits/*.svg` were deleted once nothing referenced them, and `sharp` and `delaunator` went with them: they were devDependencies of that script alone, and CI was installing sharp on every run for a generator no page and no test called. Git recovers all of it, and the six card directions and five colour variants that replaced it are readable on `card-directions.html` and `card-editorial.html`.

**The Card now.** A column of a printed page, held by 1.5px of ink at the top and 1px of `--border` at the foot, with no box, no border, no shadow and no tint at rest. Hover fades one sheet of `--surface` in behind the column and lifts it, so elevation is a state and never a resting decoration, which is the flat-by-default rule the trading card broke. Anatomy in source order: `.tt-seat`, `.tt-card` with its `::before` sheet, `.tt-type`, `.tt-name` with `.tt-setglyph`, `.tt-art` holding `.tt-ink` and `.tt-portrait`, then `.tt-role` with `.tt-company`. Retired with the trading card: the 2px ink border, `--ink-shadow-rest`, `--ink-shadow-hover`, the paper grain and `--grain-opacity`, the foil band with `--foil-opacity` and the two foil channels, the inner bevel, the collector foot line, and the fixed 300px width.

**The grade.** One chain, `--photo-grade`, through which every instructor photograph passes, and no per-person colour anywhere. The three sources were shot in three rooms under three lights, and a press run fixes that by running every plate through the same correction, not by throwing the colour away. Per-person geometry stays, because the sources were shot at three distances and a set is a set only when the heads are one size. A guard enforces the difference: geometry may vary per instructor, colour may not.

**The duplicate word.** The eyebrow read `Instructor · Practitioner` and the foot line read `PLEPIC · INSTRUCTOR`, so the word appeared twice per card and six times across three, under a heading that already says "Every instructor has a day job shipping code". The foot line is gone. It is a trading card's collector mark, an editorial contributor page does not sign itself, and PLEPIC is redundant on plepic.com. The eyebrow stays because "Practitioner" carries the section's argument.

**Numbers that moved with the register.** Tilt caps from 16 and 20 degrees to 9 and 11, perspective from 900px to 1400px, lift from 12px to 5px. A sheet of paper flexes less than card stock, and a long perspective is what keeps a rotated photograph of a face from skewing the way a rotated illustration does not.

**What the reversal taught, now anti-patterns on design-system.html.** A photograph of a real person is not raw material for a generated treatment. Decoration that repeats per item becomes noise at three items. A resting drop shadow on every card is the flat-by-default rule being broken by habit. One grade for a set of photographs, never a correction per face.

**Still open.** The photographs were shot in three rooms, and the grade closes the colour gap but not the exposure gap: Joosep's frame is brighter and denser than Vootele's, so the eye lands on him first. No shared chain fixes that. It wants a reshoot, not CSS.
