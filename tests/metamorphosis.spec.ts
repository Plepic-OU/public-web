...
test.describe('metamorphosis hero @metamorphosis', () => {
  ...
  test('reduced motion lands on the poster, never the timeline', async () => {
    ...
    expect(await page.locator('#metaCode .meta-code-line .t').first().textContent())
      .toBe('while(task) { explore → act → verify }');
    await page.close();
  });
  ...
  test('code snippet evolves with the phases and resolves to the canonical line', async () => {
    ...
    expect(await line1.textContent()).toBe('while(task) { explore → act → verify }');
